...
class Student {
    ...
    public Student(int numSubjects) {
        this.numSubjects = numSubjects; // Use 'this' to reference the instance variable
    ...
    public double calculateSGPA() {
        ...
        for (int j = 0; j < numSubjects; j++) {
            sum += ((marks[j] / 10.0) + 1) * credits[j]; // Change to 10.0 for double division
            totalCredits += credits[j];
        }
        return totalCredits > 0 ? sum / totalCredits : 0; // Avoid division by zero
    }
}
